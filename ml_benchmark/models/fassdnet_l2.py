...
class MDA(nn.Module):
    def __init__(self, nIn, d=1, kSize=3, dkSize=3):
        ...
        self.parallel_conv3x3 = Conv(
            nIn // 2, nIn // 2, dkSize, 1, padding=1, bn_acti=True
        )

        self.parallel_ddconv3x1 = Conv(
            nIn // 2,
            nIn // 2,
            (dkSize, 1),
            1,
...
class FASSDNet(nn.Module):
    def __init__(self, weights=None, n_classes=19, alpha=2):
        ...
        self.base.append(
            ConvLayer(first_ch[0], first_ch[1], kernel=3, stride=2)
        )
        self.base.append(
            ConvLayer(first_ch[1], first_ch[2], kernel=3, stride=2)
        )
        ...
        # --- DAPF
        self.DAPF = build_DAPF(
            inplanes=prev_block_channels, alpha=alpha, BatchNorm=nn.BatchNorm2d
        )
        ...
        for i in range(n_blocks - 1, -1, -1):
            ...
            self.conv1x1_up.append(
                ConvLayer(
                    cur_channels_count, cur_channels_count // 2, kernel=1
                )
            )
